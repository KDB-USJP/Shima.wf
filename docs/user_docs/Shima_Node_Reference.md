...
Many nodes that have multiple outputs that are needed for other nodes that need large amounts of inputs with have BNDL outputs. The DeBNDLer unpacks a BNDL wire into its individual component outputs for use with non-Shima nodes. Select the BNDL type via dropdown — only matching outputs are shown. Outputs a BNDL_SYNC signal for type synchronization with the ReBNDLer.

| Direction | Name | Type | Description |
|-----------|------|------|-------------|
| Input | `bndl` | BNDL | Bundle to unpack |
| Widget | `bndl_type` | COMBO | Bundle type selector |
| Output | *(varies)* | *(varies)* | Component fields for selected type |
| Output | `sync` | BNDL_SYNC | Type sync signal for ReBNDLer |

### ReBNDLer

Packs individual components back into a BNDL wire for use with Shima nodes with BNDL inputs. Select the BNDL type via dropdown — only matching inputs are shown. Accepts a BNDL_SYNC input from a DeBNDLer to auto-match the type.

| Direction | Name | Type | Description |
|-----------|------|------|-------------|
| Input | *(varies)* | *(varies)* | Component fields for selected type |
| Input | `sync` | BNDL_SYNC | Type sync from DeBNDLer (optional) |
| Widget | `bndl_type` | COMBO | Bundle type selector |
| Output | `bndl` | BNDL | Packed bundle |
