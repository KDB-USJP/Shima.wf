    ...
    async nodeCreated(node) {
        if (node.comfyClass === "Shima.DemuxList") {
            ...
            node.onConnectionsChange = function (slotType, slotIndex, isConnected, linkInfo) {
                ...
                // --- HARVESTING LOGIC ---
                // If a new output connection is made, trace it forward to sniff combo box options
                if (slotType === LiteGraph.OUTPUT && isConnected && linkInfo) {
                    ...
                    if (targetNode && targetNode.inputs) {
                        ...
                        // We need the ACTUAL underlying widget options if the slot has been converted from a widget
                        if (targetInput && targetInput.widget && targetNode.widgets) {
                            ...
                            if (actualWidget && actualWidget.type === "combo" && actualWidget.options && actualWidget.options.values) {
                                // Harvest the array elements into a comma-separated string
                                const harvestedOpts = actualWidget.options.values.join(", ");

                                // Find our own internal options widget and update it!
