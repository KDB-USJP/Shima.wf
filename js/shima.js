...
function setupCommonsWidgets(node) {
    ...
    if (!node.properties.ue_properties) {
        node.properties.ue_properties = {
            ...
            "output_not_broadcasting": {},
        };
        // Mark as converted/active for Use Everywhere
        node.properties["ue_convert"] = true;

        // Trigger a graph change so UE picks it up if it's watching
        if (app.graph) {
            node.setDirtyCanvas(true, true);
        }
        console.log("[Shima] Enabled UE broadcasting for Shima.Commons");
    }
}
